...
db.define_table(
    ...
    # Field('tags',label="标签"),
    Field('promo_type',requires=IS_IN_SET(('促销','赠品')),label="促销类型"),
    Field('name',required=True,label="产品名称"),
    Field('description_short','text',label="产品概述"),
    Field('description_long','text',label="产品详情",widget=ckeditor.widget),
    ...
    Field('delivery',requires=IS_IN_SET(('快递','包邮',)),label="送货方式"),
...
db.define_table(
    ...
    Field('image','upload',label="产品主图片"),
    Field('list_price','double',label="市场价"),
    Field('best_price','double',label="促销价"),
